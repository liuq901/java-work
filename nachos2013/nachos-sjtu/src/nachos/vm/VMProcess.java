	...
	void loadPage(int page) {
		...
		if (!VMKernel.freePage.isEmpty())
			...
			for (int i = 0; i < 26; i++) {
				...
				ID = physic.get(ppn);
				now = (VMProcess) process.get(ID.pid);
			...
			if (now.pageTable[ID.page].dirty) {
				Swap.write(new Pair(now.pid, ID.page), memory, ppn * pageSize);
				now.swapped.add(ID.page);
			}
			now.owned.remove(ppn);
		}
		owned.add(ppn);
		physic.put(ppn, new Pair(pid, page));
		if (swapped.contains(page))
			Swap.read(new Pair(pid, page), memory, ppn * pageSize);
